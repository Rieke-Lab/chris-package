classdef (Abstract) NatImgFlashWithContrastScalor < edu.washington.riekelab.protocols.RiekeLabStageProtocol
    properties
        ...
        contrastScalor = 1 % Scalar or matrix of whole-image contrast scales in [0, 1]
        patchContrast = 'all'
    ...
    properties (Hidden)
        ampType
        contrastScalorType = symphonyui.core.PropertyType('denserealdouble', 'matrix')
        currentContrastScalor
        contrastScalorIndex
        originalImageMatrix % Normalized double image, before contrast scaling
    ...
    methods
        ...
        function prepareRun(obj)
            validateattributes(obj.contrastScalor, {'double'}, ...
                {'real', 'finite', 'nonempty', '2d', '>=', 0, '<=', 1}, ...
                class(obj), 'contrastScalor');
            obj.currentContrastScalor = [];
            obj.contrastScalorIndex = 1;
            prepareRun@edu.washington.riekelab.protocols.RiekeLabStageProtocol(obj);
            % NatImgFlashWithContrastScalor.m is in .../+chris/+protocols
            % resources is in .../+chris/+resources
            thisFile = mfilename('fullpath');
            protocolDir = fileparts(thisFile);
            chrisDir = fileparts(protocolDir);
            resourcesDir = fullfile(chrisDir, '+resources/');
            ...
            if fileId == -1
                error('NatImgFlashWithContrastScalor:ImageOpenFailed', ...
            ...
            if numel(img) ~= 1536 * 1024 || max(img(:)) <= 0
                error('NatImgFlashWithContrastScalor:InvalidImage', ...
                    'Image must contain 1536 x 1024 pixels and a positive maximum.');
            end
            img = double(img);
            ...
            library = load(fullfile(resourcesDir, [obj.currentStimSet '.mat']), 'imageData');
            imageData = library.imageData;
            ...
            obj.patchLocations = zeros(2, obj.noPatches);
            obj.patchLocations(1,1:obj.noPatches) = xLoc(pullInds); %in VH pixels
            obj.patchLocations(2,1:obj.noPatches) = yLoc(pullInds);

        end

        function setContrastScalor(obj, value)
            % Always scale the original image, never the previously scaled image.
            if isequal(obj.currentContrastScalor, value)
                return;
            end
            [obj.contrastImage, obj.wholeImageMatrix] = ...
                edu.washington.riekelab.chris.utils.scaleNaturalImageContrast( ...
                obj.originalImageMatrix, obj.backgroundIntensity, value);
            obj.currentContrastScalor = value;
        end

        function prepareEpoch(obj, epoch)
            prepareEpoch@edu.washington.riekelab.protocols.RiekeLabStageProtocol(obj, epoch);

            epoch.addParameter('currentContrastScalor', obj.currentContrastScalor);
            epoch.addParameter('contrastScalorIndex', obj.contrastScalorIndex);
