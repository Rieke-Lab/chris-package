classdef SplitFieldAlternatingProtocol < edu.washington.riekelab.protocols.RiekeLabStageProtocol
    ...
    methods
        ...
        function p = createPresentation(obj)
            ...
            leftField.position = [canvasSize(1)/2 - (rightWidth + halfGapWidth), canvasSize(2)/2];
            ...
            % Create middle gap rectangle (fixed at minimum intensity)
            gapIntensity = min(obj.leftMeanIntensity);
            middleGap = stage.builtin.stimuli.Rectangle();
            middleGap.position = [canvasSize(1)/2, canvasSize(2)/2];
            middleGap.size = [gapSizePix, totalHeight];
            middleGap.color = [gapIntensity, gapIntensity, gapIntensity];
            p.addStimulus(middleGap);
            
            % Create right field rectangle
            rightField = stage.builtin.stimuli.Rectangle();
            rightField.position = [canvasSize(1)/2 + (leftWidth + halfGapWidth), canvasSize(2)/2];
            ...
            % Controller for left field intensity (constant throughout epoch)
            leftIntensityController = stage.builtin.controllers.PropertyController(leftField, 'color', ...
                @(state)getLeftFieldIntensity(obj));
            p.addController(leftIntensityController);
            ...
            function c = getLeftFieldIntensity(obj)
                % Return constant intensity for the left field during this epoch
                intensity = obj.currentLeftMean;
